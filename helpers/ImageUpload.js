  ...
  promiseAllResize(file, i, prop = '', filenameParam = this.filename) {
    Promise.all(
      this.sizes.map((size) => {
        ...
        this.imageFilename = `${size.path}-${
          filenameParam.split(/\.([^.]+)$/)[0]
        }${i != undefined ? `-${i}` : ''}.${this.imageExt}`;
        this.imageUploadPath = this.uploadPath;
        fs.mkdirsSync(this.imageUploadPath);
        this.filesUploaded.push({
          originalname: file.originalname,
          ...(prop && { field: prop }),
          filename: this.imageFilename,
          path: `${this.fileUrl}/${this.imageFilename}`,
        });
